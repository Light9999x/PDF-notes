...
export function ImageGallery({doc,onImport,onPick,onDrag,onRemove,onCutout,busy}:{onRemove:(hash:string)=>Promise<void>;onCutout:(hash:string)=>void;doc:NoteDocument;onImport:(file:File)=>Promise<void>;onPick:(hash:string)=>void;onDrag:(hash:string,phase:'move'|'up'|'cancel',p:Point)=>void;busy:boolean}){
  const input=useRef<HTMLInputElement>(null),[error,setError]=useState(''),[importing,setImporting]=useState(false),hashes=galleryHashes(doc),[removing,setRemoving]=useState(false);
  async function remove(hash:string){setRemoving(true);setError('');try{await onRemove(hash);}catch(e){setError(String(e));}finally{setRemoving(false);}}
  return <><button disabled={busy||importing||removing} onClick={()=>input.current?.click()}>選取檔案</button><input ref={input} type="file" accept="image/png,image/jpeg" hidden onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file){setError('');setImporting(true);void onImport(file).catch(e=>setError(String(e))).finally(()=>setImporting(false));}}}/>{error&&<p role="alert">{error}</p>}{!hashes.length&&<p>匯入 PNG／JPG，拖到頁面或點選後放置。</p>}<div className="gallery-grid">{hashes.map(h=>doc.assets[h]&&<div className="gallery-item" key={h}><Thumbnail disabled={busy||importing||removing} asset={doc.assets[h]} onPick={onPick} onDrag={onDrag}/><ActionMenu label={'圖片素材 '+h.slice(0,8)}><button disabled={busy||removing} onClick={()=>onCutout(h)}>去背副本</button><button disabled={busy||removing} onClick={()=>void remove(h)}>從圖片庫移除</button></ActionMenu></div>)}</div></>;
}
