...
import { ActionMenu, Disclosure, Button, Dialog, EmptyState, Field, Notification, StatusBadge } from './components/ui';
...
export default function App() {
  ...
  return <><div className="app-shell">
    ...
    <main className={current?'reader-main':'main-content'}>
      ...
      <section className="settings-card full"><Disclosure title="支援範圍與待驗收項目"><div className="support-row"><span><Check size={17}/>PDF、PNG／JPG 本機匯入</span><span><Check size={17}/>四類註記與兩種匯出</span><span><Check size={17}/>.pdfnote 完整備份</span></div><p className="muted">此版本尚不支援 Word 轉換。未提供 OCR；掃描頁不能搜尋文字。Windows／Galaxy S24+ 實機、觸控筆、Google 帳號端到端同步驗收仍待完成。</p></Disclosure></section></div></>}
      </>}
    </main>
