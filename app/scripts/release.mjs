...
files['README.md']=strToU8((await fs.readFile('README.md','utf8')).replaceAll('(../prompts/','(prompts/').replaceAll('(../README.md)','(GETTING_STARTED.md)'));
files['GETTING_STARTED.md']=strToU8((await fs.readFile('../README.md','utf8')).replaceAll('(app/','('));
...
for(const directory of ['docs','prompts'])for(const file of await fs.readdir(directory==='prompts'?'../prompts':directory)){if(file.endsWith('.md'))files[directory+'/'+file]=strToU8((await fs.readFile(path.join(directory==='prompts'?'../prompts':directory,file),'utf8')).replaceAll('(../../prompts/','(../prompts/').replaceAll('(../../AGENTS.md)','(../AGENTS.md)').replaceAll('(../../.agents/','(../.agents/').replaceAll('(../app/','(../'));}
